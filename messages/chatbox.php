...
<div class="messages_container">
    ...
    <div style="text-align: <? echo lang('textAlign'); ?>">
    	<div class="messages" style="min-width:140px;max-width:700px;height:410px;">
			...
			<?php 
				//$uid_session = $_SESSION["Username"];
				$getS = $conn->prepare("SELECT * FROM signup where (id = :myid)");
				$getS->bindParam(':myid',$myid,PDO::PARAM_INT);
				$getS->execute();
				$getSCout = $getS->rowCount();
				// if selected messages more than zero [0] do this, If not >> do [else] code
				if ($getSCout > 0) {
				while ($s_row = $getS->fetch(PDO::FETCH_ASSOC)) {
					$me_s = $s_row['id'];
				}
				}	
...
";
            ...
            <form method="POST" action='insert_msg.php?myid="<?php echo $me_s;?>&&
			yid=<?php echo $uid;  ?>'>
    		<div class="m_SendField_box">
    			<div class="m_SendField">
					...
					<button  name="envoi"  type="submit">Send</button></span>
